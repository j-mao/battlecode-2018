		...
		for (int i = 0;i < numunits;i++)
		{
			if (units.get(i).unitType() == UnitType.Ranger) {
				...
				*/
				...
				// testing with gp ranger
				gpRunRanger(units.get(i));

			} else if (units.get(i).unitType() == UnitType.Worker) {

				if (false && roundNum > 250) {
					// jerry worker late game
					if (!units.get(i).location().isInGarrison() && !units.get(i).location().isInSpace())
					{
					...
					// gp worker early game
					gpRunEarthWorker(units.get(i));
				}
			}
		}
	}

	private static void gpRunEarthWorker (Unit unit) {
